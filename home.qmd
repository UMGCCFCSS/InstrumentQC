...
# Updating Data
walk(.x=TheList, MainFolder=MainFolder, .f=Luciernaga:::LevyJenningsParse, Maintainer=FALSE)
...
InstrumentStatus1 <- InstrumentStatus %>%
  mutate(Status = case_when(
    QCStatus == "Green" ~ "Pass",
    QCStatus == "Orange" ~ "Caution",
    QCStatus == "Red" ~ "Failing",
    TRUE ~ NA_character_
  ))

InstrumentStatus1 <- InstrumentStatus1 %>%
  mutate(StatusTag = case_when(
    QCStatus == "Green" ~ "success",
    QCStatus == "Orange" ~ "warning",
    QCStatus == "Red" ~ "danger",
    TRUE ~ NA_character_
  ))

TheStatus3L <- InstrumentStatus1 %>% filter(Instrument %in% "3L") %>% pull(Status)
TheColor3L <- InstrumentStatus1 %>% filter(Instrument %in% "3L") %>% pull(StatusTag)
TheStatus4L <- InstrumentStatus1 %>% filter(Instrument %in% "4L") %>% pull(Status)
TheColor4L <- InstrumentStatus1 %>% filter(Instrument %in% "4L") %>% pull(StatusTag)
TheStatus5L <- InstrumentStatus1 %>% filter(Instrument %in% "5L") %>% pull(Status)
TheColor5L <- InstrumentStatus1 %>% filter(Instrument %in% "5L") %>% pull(StatusTag)
TheStatusCS <- InstrumentStatus1 %>% filter(Instrument %in% "CS") %>% pull(Status)
TheColorCS <- InstrumentStatus1 %>% filter(Instrument %in% "CS") %>% pull(StatusTag)
```
...
#Utility_Patchwork(x=PDFPlots, filename="3LPlots", returntype="pdf", outfolder="C:/Users/12692/Desktop", thecolumns=1)
```
...
::: {.card title="LaserPower"}
:::

::: {.card title="LaserScaling"}
:::
...
#Utility_Patchwork(x=PDFPlots, filename="4LPlots", returntype="pdf", outfolder="C:/Users/12692/Desktop", thecolumns=1)
```
...
#Utility_Patchwork(x=PDFPlots, filename="5LPlots", returntype="pdf", outfolder="C:/Users/12692/Desktop", thecolumns=1)
```
